...
export default function Schedule() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://mediflow-cse416.onrender.com/userID/${userId}`, {
        headers: {
          Authorization: "Bearer " + sessionStorage.getItem("token"),
        },
      })
  ...
  useEffect(() => {
    ...
    setFullCalendar(
      userAppointments.flatMap((appointmentId) => {
        let appointmentItem = appointmentsList.find(
          (item) => item._id === appointmentId
        );
        if (!appointmentItem) return [];

        return appointmentItem.procedures
          .map((procedure) => {
            const patient = usersList.find(
              (patient) => patient._id === appointmentItem.patient
            );
            if (!patient) return null;

            //console.log(procedure.scheduledStartTime)

            const start = moment(procedure.scheduledStartTime)
              .tz("America/New_York")
              .format();
            const end = moment(procedure.scheduledEndTime)
              .tz("America/New_York")
              .format();
