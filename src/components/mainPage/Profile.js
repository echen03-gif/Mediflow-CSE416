...
import { Avatar, Button, Typography, Box, Card, CardContent, Grid, List, ListItem, ListItemText, Pagination, CardActions, Modal } from '@mui/material';
...
function Profile() {
        ...
        {selectedProcedure && (
            <>
                ...
                <Typography variant="subtitle2" align="center" sx={{ mt: 1 }}>
                    Date: {new Date(selectedProcedure.scheduledStartTime).toLocaleDateString()}
                </Typography>
                <Typography variant="subtitle2" align="center">
                    Time: {new Date(selectedProcedure.scheduledStartTime).toLocaleTimeString()}
                </Typography>
  ...
  useEffect(() => {
    ...
    const fetchAppointments = async () => {
      const userId = sessionStorage.getItem('user');
      const response = await axios.get(`https://mediflow-cse416.onrender.com/profileappt`, {
        headers: {
          'Authorization': 'Bearer ' + sessionStorage.getItem('token')
        }
      });
  ...
  return (
    <Grid container spacing={3} sx={{ p: 3, height: '100vh', overflow: 'auto' }}>
      <Grid item xs={12} md={4}>
        ...
        <Card>
          <CardContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <Typography variant="h6" color="textSecondary" sx={{ mb: 2 }}>Edit Profile</Typography>
            <Avatar
              src={`https://mediflow-cse416.onrender.com/uploads/${sessionStorage.getItem('pfp')}`}
        ...
        <Card sx={{ mb: 3, width: '100%' }}>
          <CardContent>
              ...
              {paginatedAppointments.map((appointment) => (
                appointment.procedures.map((procedure) => {
                  if (procedure.staff.some(staff => staff.toString() === user._id)) {
                    return (
                      <ListItem button key={`${appointment._id}-${procedure._id}`} onClick={() => handleOpenModal(appointment, procedure)}>
                        <ListItemText 
                          primary={`Procedure: ${procedure.procedure.name} on ${new Date(procedure.scheduledStartTime).toLocaleDateString()}`}
            ...
            <CardActions sx={{ justifyContent: 'center' }}>
              <Pagination count={Math.ceil(appointments.length / 5)} page={page} onChange={handlePageChange} color="primary" />
            </CardActions>
          </CardContent>
        </Card>
        <Card sx={{ mb: 3, width: '100%' }}>
              ...
              <Box key={day} sx={{ mb: 1 }}>
                ...
                {shifts.map((shift, index) => (
                  <Typography key={index} variant="body2" color="textSecondary">
                    {new Date(`1970-01-01T${shift.start}Z`).toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })} -  {new Date(`1970-01-01T${shift.end}Z`).toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })}
                  </Typography>
                ))}
              </Box>
            ))}
