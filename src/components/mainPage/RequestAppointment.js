...
export default function RequestAppointment() {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://mediflow-cse416.onrender.com/equipmentHead", {
        headers: {
          Authorization: "Bearer " + sessionStorage.getItem("token"),
        },
      })
      .then((res) => {
        setEquipmentHeadList(res.data);
      });
  ...
  const handleSubmit = async (e) => {
    ...
    const procedures = Object.entries(staffSelections).map(
      ([
        procedureId,
        { staff, equipment, scheduledStartTime, scheduledEndTime, roomId },
      ]) => {
        let procedure = procedureList.find(
      ...
      for (let i = 0; i < usersList.length; i++) {
        ...
        for (let equipmentName of equipmentListName) {
          let equipmentHead = equipmentHeadList.find(
            (equipment) => equipment.name === equipmentName
          );

          for (let equipmentId of equipmentHead.equipment) {
            let checkEquipment = equipmentList.find(
              (equipmentSearch) => equipmentSearch._id === equipmentId
            );
            ...
            if (
              ...
              checkEquipment.type === procedure.staffType
            ) {
        ...
        if (
          selectedStaff.length !== numStaff ||
          selectedEquipment.length !== equipmentListName.length
        ) {
        ...
        scheduledStartTime = moment
          .tz(scheduledStartTime, "YYYY-MM-DDTHH:mm:ss.SSS", "America/New_York")
          .utc()
          .format();
        scheduledEndTime = moment
          .tz(scheduledEndTime, "YYYY-MM-DDTHH:mm:ss.SSS", "America/New_York")
          .utc()
          .format();
    ...
    if (!checkAvaliability) {
      setNotification(
        `There are not enough available staff, room, or equipment at the selected time for ${NonAvaliableProcedures.join(
          ", "
        )}`
      );
      return;
    ...
    let newAppointment = await axios
      .post(
        "https://mediflow-cse416.onrender.com/requestAppointment",
        {
          patient: patientUser,
    ...
    await Promise.all([
      ...Array.from(uniqueStaffIds).map((staffId) =>
        axios.put(
          "https://mediflow-cse416.onrender.com/changeStaffAppointment",
          {
            staffName: staffId,
      ...
      ...Array.from(uniqueEquipmentIds).map((equipmentId) =>
        axios.put(
          "https://mediflow-cse416.onrender.com/changeEquipmentAppointment",
          {
    ...
    await Promise.all(
      Array.from(uniqueRoomIds).map((room) => {
        console.log(room);
        return axios.put(
          "https://mediflow-cse416.onrender.com/changeRoomAppointment",
          {
            roomObject: room,
            appointment: newAppointment.data,
          },
  ...
  return (
    <Container
      maxWidth="lg"
      sx={{ display: "flex", mt: 4, height: "90vh", overflow: "auto" }}
    >
              ...
              <Box key={procedure}>
                ...
                <Typography variant="h6">Estimated Duration: {procedureList.find(item => item._id === procedure).estimatedDuration} mins</Typography>
                  ...
                  <TextField
                    ...
                    onChange={(e) =>
                      handleChange(procedure, "scheduledStartTime")(
                        e,
                        e.target.value
                      )
                    }
